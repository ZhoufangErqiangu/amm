...
impl Pack for AmmPool {
    ...
    fn unpack_from_slice(src: &[u8]) -> Result<Self, ProgramError> {
        ...
        let (
            ...
            fee_receiver_4_buf,
            fee_receiver_5_buf,
        ) = array_refs![src, 1, 1, 8, 8, 8, 8, 8, 8, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32];

        Ok(AmmPool {
            status: u8::from_le_bytes(*status_buf),
            nonce: u8::from_le_bytes(*nonce_buf),
            k: f64::from_le_bytes(*k_buf),
            ...
            fee_receiver_1: Pubkey::new_from_array(*fee_receiver_1_buf),
    ...
    fn pack_into_slice(&self, dst: &mut [u8]) {
        ...
        let (
            ...
            fee_receiver_4_buf,
            fee_receiver_5_buf,
        ) = mut_array_refs![dst, 1, 1, 8, 8, 8, 8, 8, 8, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32];
        *status_buf = self.status.to_le_bytes();
        *nonce_buf = self.nonce.to_le_bytes();
        *k_buf = self.k.to_le_bytes();
