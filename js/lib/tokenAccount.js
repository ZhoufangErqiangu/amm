...
export async function getTokenAccountData(connection, tokenKey) {
    ...
    if (temp.value) {
        ...
        return {
            code: 1, msg: 'get token account data ok', data: {
                publicKey: tokenKey,
                owner: info.owner,
                mint: info.mint,
                amount,
...
export async function getTokenAccountMaxAmount(connection, wallet, mintKey) {
    ...
    let mintAcc = new PublicKey(mintKey);
    // find token accounts
    let res = await connection.getTokenAccountsByOwner(walletAcc, { mint: mintAcc });
    if (res.value.length == 1) {
        // user has only one token aacount
        let res1 = await getTokenAccountData(connection, res.value[0].pubkey.toBase58());
        if (res1.code == 1) {
            return { code: 1, msg: 'user has only one token account', data: res1.data };
        } else {
            return res1;
        }
        ...
        if (accounts.length > 0) {
            let amounts = accounts.map((e) => { return e.amount; });
            let maxAmount = Math.max(...amounts);
            let maxIndex = amounts.indexOf(maxAmount);
            if (maxIndex != -1) {
                return { code: 1, msg: 'find token account ok', data: accounts[maxIndex] };
            } else {
                return { code: 0, msg: 'can not find amount max token account', data: null };
            }
        } else {
            return { code: 0, msg: 'can not find amount max token account', data: null };
        }
...
export async function getMintData(connection, mintKey) {
    ...
    if (temp.value) {
        ...
        return {
            code: 1, msg: 'get mint data ok', data: {
                publicKey: mintKey,
